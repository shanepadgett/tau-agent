...
`injectFiles()` reads or outlines files and injects them as visible `tau.file` messages, in request order. `prepareFileInjection()` builds the same messages without sending them, for callers that append to another session themselves.
