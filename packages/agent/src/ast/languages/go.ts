...
function visibilityFor(name: string): Visibility {
	// Go has no file-private level: lowercase is package-visible, the analogue of pub(crate).
	return goExported(name) ? "public" : "internal";
}
...
/** Embedded interface names in an interface body (`type_elem` children). */
function interfaceEmbeds(iface: Node): string[] {
	const out: string[] = [];
	for (const node of iface.namedChildren) {
		if (node.type !== TYPE_ELEM) continue;
		const text = node.text.trim();
		// Type sets (`~int | string`) are constraints, not embedded interfaces.
		if (text.includes("|") || text.startsWith("~")) continue;
		const bare = text.includes(".") ? (text.split(".").pop() ?? text) : text;
		if (/^[A-Za-z_]\w*$/u.test(bare)) out.push(bare);
	}
	return out;
}
			...
			const decl = applyDoc(
				{
					...
					calls: [],
					// Embedded interfaces are heritage: `implementations` follows them.
					bases: interfaceEmbeds(typeNode),
				},
				doc,
			);
			return [decl];
