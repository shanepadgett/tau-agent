...
function openFence(text: string): FenceState | undefined {
	const fence = text.match(OPEN_FENCE);
	const fenceMarker = fence?.[2];
	if (fenceMarker === undefined) return undefined;
	const char = fenceMarker[0] === "~" ? "~" : "`";
	return { char, length: fenceMarker.length };
}

function parseAtxHeading(text: string): { depth: number; name: string } | undefined {
	const heading = text.match(ATX_HEADING);
	if (heading === null) return undefined;
	const hashes = heading[1];
	if (hashes === undefined) return undefined;
	const name = (heading[2] ?? "").trim();
	if (name.length === 0) return undefined;
	return { depth: hashes.length, name };
}

function makeHeadingDecl(info: LineInfo, name: string, parent: OpenHeading | undefined): Decl {
...
export function extractMarkdown(source: string): ExtractResult {
	...
	let inFence: FenceState | undefined;

	for (const info of lines) {
		if (inFence !== undefined) {
			if (closesFence(info.text, inFence)) inFence = undefined;
			continue;
		}

		const opened = openFence(info.text);
		if (opened !== undefined) {
			inFence = opened;
			continue;
		}

		const heading = parseAtxHeading(info.text);
		if (heading !== undefined) pushHeading(lines, info, heading, stack, roots);
	}
