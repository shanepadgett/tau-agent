...
const COMPACT_SIGNATURE_MAX = 80;

/** One-line signature for row output: whitespace collapsed, capped. */
export function compactSignature(text: string): string {
	const collapsed = text.replace(/\s+/gu, " ").trim();
	return collapsed.length > COMPACT_SIGNATURE_MAX ? `${collapsed.slice(0, COMPACT_SIGNATURE_MAX - 3)}...` : collapsed;
}
