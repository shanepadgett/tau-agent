...
function formatFileRow(row: ImpactFileRow, cwd: string, showDepth: boolean): string {
	...
	if (row.target.kind === "internal") return `  ${depth}${formatPathForDisplay(row.target.path, cwd)}`;
	if (row.target.kind === "package") {
		return `  ${depth}${formatPathForDisplay(row.target.dir, cwd)}/ (${row.target.fileCount} files)`;
	}
	return `  ${depth}${row.target.id}`;
}
...
function formatSection(section: ImpactSection, cwd: string): string[] {
	...
	if (fileRows.length > 0) {
		const internal = fileRows.filter((row) => row.target.kind !== "external");
		const external = fileRows.filter((row) => row.target.kind === "external");
