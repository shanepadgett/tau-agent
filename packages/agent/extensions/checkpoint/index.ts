...
import { setCheckpointRowsVisible } from "../../shared/checkpoint-visibility.ts";
...
import { CHECKPOINT_SYSTEM_GUIDANCE, formatCheckpointMessage } from "./prompt.ts";
import checkpointSettings from "./settings.ts";
...
export default function checkpointExtension(pi: ExtensionAPI): void {
	...
	pi.on("session_start", async (_event, ctx) => {
		const settings = await loadTauExtensionSettings(ctx, checkpointSettings);
		budget.configure(settings.checkpointTokenLimit);
		setCheckpointRowsVisible(settings.showToolRows);
	});
	pi.on("session_shutdown", () => {
		setCheckpointRowsVisible(false);
		resetBudget();
	});
...
function sendCheckpointBudgetNotice(pi: ExtensionAPI, level: CheckpointBudgetNoticeLevel): void {
	pi.sendMessage({
		customType: "tau.checkpoint",
		content: checkpointBudgetMessage(level),
		display: false,
		details: {
			v: 1,
			kind: "checkpoint.checkpoint-budget",
			source: "checkpoint",
			level,
		},
	});
	...
	switch (level) {
		case 50:
			return formatCheckpointMessage(
				"budget",
				"Context usage reached 50% of the configured checkpoint limit. Continue current work and checkpoint when convenient.",
				{ level: "50" },
			);
		case 75:
			return formatCheckpointMessage(
				"budget",
				"Context usage reached 75% of the configured checkpoint limit. Run checkpoint soon. At 100%, non-checkpoint tool calls will be blocked until checkpoint succeeds.",
				{ level: "75" },
			);
		case 100:
			return formatCheckpointMessage(
