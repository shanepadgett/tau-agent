...
```text
git • provider/model (thinking)                S $0.18 · D $2.43
cwd • session                                  footer items
```

Other Tau extensions can publish bottom-right items with `setTauFooterItem()` from `shared/events.ts (or public event bus docs)`.
