...
function packTarget(state: PackState, snap: DeclSnap): void {
	const left = state.budget - state.used;
	const preferBody = !bodyUnavailable(snap);
	if (preferBody) {
		const body = entryFrom(snap, "body", []);
	...
	const flags: ContextFlag[] = ["target_omitted"];
