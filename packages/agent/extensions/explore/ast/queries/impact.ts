...
import type { ExploreFileGraph, FileGraphEdge } from "../graph/file-graph.ts";
import {
	queryRelationships,
...
function fileRowsFromEdges(edges: readonly FileGraphEdge[]): ImpactFileRow[] {
	...
	for (const edge of edges) {
		if (rows.length >= IMPACT_SECTION_LIMIT) break;
		if (edge.kind === "internal") {
			rows.push({ kind: "file", path: edge.to, externalId: undefined, depth: 1 });
		} else {
			rows.push({ kind: "file", path: undefined, externalId: edge.id, depth: 1 });
		}
	}
	return rows;
}
...
export async function queryImpact(args: ImpactQueryArgs): Promise<ImpactResult> {
		...
		try {
			...
			if (rows.length > 0) {
				sections.push({
					...
					rows,
					limit: IMPACT_SECTION_LIMIT,
					omitted: edges.length > IMPACT_SECTION_LIMIT,
				});
			}
		} catch (error) {
	...
	if (wantDependents) {
		...
		try {
			...
			const importers = importerHits.slice(0, IMPACT_SECTION_LIMIT).map(
				(hit): ImpactFileRow => ({
					kind: "file",
					path: hit.path,
			...
			if (importers.length > 0) {
				sections.push({
					...
					rows: importers,
					limit: IMPACT_SECTION_LIMIT,
					omitted: importerHits.length > IMPACT_SECTION_LIMIT,
				});
			}

			if (depth >= 2) {
				const transitiveHits = reverse.hits.filter((hit) => hit.depth >= 2 && hit.depth <= depth);
				const transitive = transitiveHits.slice(0, IMPACT_SECTION_LIMIT).map(
					(hit): ImpactFileRow => ({
						kind: "file",
						path: hit.path,
						externalId: hit.externalId,
				...
				if (transitive.length > 0) {
					sections.push({
						...
						rows: transitive,
						limit: IMPACT_SECTION_LIMIT,
						omitted: transitiveHits.length > IMPACT_SECTION_LIMIT || reverse.resultLimitReached,
					});
				}
			}
		} catch (error) {
