	...
	return createAstToolPreviewWidget(tui, cwd, theme, "references", [
		{
			...
			args: {
				path: scope,
				targetPath: "packages/agent/src/ast/engine.ts",
			...
			result: [
				"createExploreEngine  function  packages/agent/src/ast/engine.ts:212",
				"",
	...
	return createAstToolPreviewWidget(tui, cwd, theme, "implementations", [
		{
			...
			result: ["ExploreEngine  typeAlias  packages/agent/src/ast/engine.ts:39", "No relationship sites."].join("\n"),
			declarationCount: 0,
			returnedBytes: 116,
		},
	]);
