...
It keeps the core prompt stable for cache reuse, captures runtime context once per session, and uses slash commands to add branch-scoped guide context without rewriting the base prompt.
...
Guide markers are human-visible history. They are filtered out of model context. Guide removal is branch-scoped filtering, not session-history deletion.
