...
All scripts under `docs/plans/fs-tool-research/`. Reports under `docs/plans/fs-tool-research/reports/` as Markdown + CSV/JSON.
...
Start metrics, not scaffolding.

1. Create `docs/plans/fs-tool-research/repo-size-profile.py`.
2. Create `docs/plans/fs-tool-research/context-pack-pricing-sim.py`.
3. Fill reports under `docs/plans/fs-tool-research/reports/`.
4. Then `turn-cost-sim.py` and `cache-layout-sim.py`.

Do not choose extension name until measured shape is clearer.
