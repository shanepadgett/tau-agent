...
- Directory walks via `scan.ts` budgets. Note: matching needs live trees — for this tool, parse → match → `tree.delete()` per file; do not route through the IR cache (IR has no tree).
...
Pattern matching per language family (TS, Go, Rust minimum): plain pattern, `$NAME`, `$$$ARGS`, no-match, language mismatch error, directory-without-language error, oversized pattern error, budget trip.
