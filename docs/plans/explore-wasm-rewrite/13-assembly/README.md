...
1. Audit `packages/agent/extensions/explore/index.ts` as composition root. As of task 11 it already builds engine + graph once, lifecycle subscriptions, and registers **all 12** tools: `outline`/`show`/`discover`/`ast_search`/`deps`/`reverse_deps`/`callers`/`callees`/`references`/`implementations`/`impact`/`context`. **Do not rebuild or re-register tools.** Remaining gap before/with this task: task 12 read hook + autoread only. Compact lifecycle per `explore-specs/cross/system.md`. Pi keeps `ls`/`find`/`grep`/`read`. `ast_search` uses engine `searchInSource` (`@ast-grep/wasm` + pinned grammars; see `11-ast-search/decision.md`) — assembly does not touch matcher internals.
...
- After `/reload`, full harness smoke per [`../LIVE-PROVE.md`](../LIVE-PROVE.md): recursive `outline`, `show`, `discover`, `ast_search` (one plain/`$`/`$$$`/empty per claimed language family is enough — full 11 prove already done), graph/relationship tools, Pi `read` large-file outline substitution (if 12 landed), `impact`, `context` — each on corpus scopes covering the languages still claimed, plus optional monorepo TS.
