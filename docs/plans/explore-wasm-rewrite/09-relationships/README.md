...
The four relationship tools over one shared pipeline. Staged. There is **no** `tests` tool (`stripped.md`).
...
TS fixture repo exercising: direct calls, method calls, type uses, re-export, an ambiguous duplicate name, callee extraction from a body, implementations via `implements`/`extends`. Go fixture for interface implementation heuristics. Empty results. Budget trip.
