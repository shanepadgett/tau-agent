...
5. Signature = exact slice of real source. All IR offsets are UTF-16 code units into the decoded string (plan README decision 12): emit `node.startIndex`/`node.endIndex` as-is, slice with `source.slice(...)`, never convert to bytes, no `Buffer` outside engine hashing.
