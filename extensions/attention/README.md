...
- Listens for shared event `tau:attention` so other extensions can request the same notification.
...
```ts
pi.events.emit("tau:attention", { title: "Tau", body: "Ready for input" });
```
